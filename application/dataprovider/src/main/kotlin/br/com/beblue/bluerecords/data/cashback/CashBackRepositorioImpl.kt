...
open class CashBackRepositorioImpl(private val jdbcTemplate: JdbcTemplate) : CashBackRepositorio {

    private val SELECT_CASHBACK = " SELECT id_cashback,id_genero,dia_semana,porcentagem_cash_back " +
            " FROM cashback_tbl" +
            " where id_genero = ? and dia_semana = ?; "

    @Cacheable(value = ["buscarCashBack"], key="{#idGenero,#diaSemana}", cacheManager = "cacheManagerWithTenMinutes")
